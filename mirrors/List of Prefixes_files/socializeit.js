var socializeit=function(options){function hasClass(el,className){var regexp=new RegExp("(^|\\s)"+className+"(\\s|$)");return regexp.test(el.className);}
function addClass(el,className){if(!hasClass(el,className)){el.className=el.className+" "+className;}}
function removeClass(el,className){var re=new RegExp("(^|\\s)"+className+"(\\s|$)");while(re.test(el.className)){el.className=el.className.replace(re,' ');}
el.className=el.className.replace(/^\s+/,'').replace(/\s+$/,'');}
if(!options||!options.domain){throw new Error('Error: must pass socializeit a domain (e.g. socializeit({domain:"www.example.com"})).')}
if(typeof options.share_horizontal_label=="undefined"){options.share_horizontal_label='Share this page:';}
if(typeof options.share_sticky_label=="undefined"){options.share_sticky_label='Share';}
if(typeof options.whatIsThisLabel=="undefined"){options.whatIsThisLabel='What\'s this?';}
if(!options.pack)options.pack=1;var d=document,l=d.location,e=encodeURIComponent,u=e(l.href),t=e(d.title),appId,appIdNode=d.querySelector('meta[property="fb:app_id"]');if(appIdNode){appId=Number(appIdNode.getAttribute('content'));}
var sitesConfig={'facebook':{name:'Facebook',color:'#3b579d',iconClassName:'fab fa-facebook-square fa-2x fa-fw',url:'https://www.facebook.com/sharer.php?u='+u+'&t='+t},'twitter':{name:'Twitter',color:'#28a9e0',iconClassName:'fab fa-twitter-square fa-2x fa-fw',url:'https://twitter.com/intent/tweet?text=Reading%20about%20this:%20'+t+'%20-%20'+u},'pinterest':{name:'Pinterest',color:'#CD1D1F',iconClassName:'fab fa-pinterest-square fa-2x fa-fw',url:'https://assets.pinterest.com/js/pinmarklet.js',includeScript:true},'tumblr':{name:'Tumblr',color:'#35465c',iconClassName:'fab fa-tumblr-square fa-2x fa-fw',url:'https://www.tumblr.com/share/link?url='+u+'&name='+t},'reddit':{name:'Reddit',color:'#ff4500',iconClassName:'fab fa-reddit-square fa-2x fa-fw',url:'https://reddit.com/submit?url='+u+'&title='+t},'whatsapp':{name:'WhatsApp',color:'#25D366',iconClassName:'fab fa-whatsapp-square fa-2x fa-fw',url:'https://api.whatsapp.com/send?text='+u+''},'fbmessenger':{name:'Messenger',color:'#0078FF',iconClassName:'fab fa-facebook-messenger fa-2x fa-fw',url:'https://www.facebook.com/dialog/send?app_id='+appId+'&link='+u+'&redirect_uri='+u+'',windowHeight:500,windowWidth:850}};var sites=[];if(options.pack==1){sites=[sitesConfig.facebook,sitesConfig.twitter,sitesConfig.pinterest,sitesConfig.tumblr,sitesConfig.reddit,sitesConfig.whatsapp];if(appId){sites.push(sitesConfig.fbmessenger);}}
else
...
var buttonsInRow=Math.ceil((sites.length/rows));var colspan=buttonsInRow+1;var rowspan='rowspan="'+rows+'"';var socit_class='socializeIt'+' '+(pack==2?'socializeIt-2':'socializeIt-8');var inlineStyles='border: 1px solid #ccc;border-collapse:separate; padding:2px;max-width:480px;';if(options.background_color){inlineStyles+='background-color:'+options.background_color;}else{if(options.display_variant){inlineStyles+='background-color: #fff;';}}
var id='socializeit'+(new Date()).getTime()+Math.floor(Math.random()*1e7);socializeit.payItForward=function(){if(d.getElementById(id).style.display=='block'){d.getElementById(id).style.display='none';}else{d.getElementById(id).style.display='block';}};if(options.version===2){socit_class+=' socializeIt-responsive';html='<div class="'+socit_class+'" style="'+inlineStyles+'">';if(options.display_variant){html+='<div class="shareHeader">'+options.share_sticky_label+'</div>';}else{html+='<div class="shareHeader">';html+='<strong class="ss-shareLabel" style="font-size: 16px;">'+options.share_horizontal_label+'</strong>';if(options.whatIsThisUrl){html+='<div id="whatsthis" style="font-size:11px;">'+
...
'</div>';}
html+='</div>';}
html+='<ul class="socialMedia">';for(var i=0,ilen=sites.length;i<ilen;i+=1){var site=sites[i].name;var url=sites[i].url;var windowHeight=sites[i].windowHeight||500;var windowWidth=sites[i].windowWidth||700;var includeScript=sites[i].includeScript;var onClickAction;if(sites[i].onClickCallback){onClickAction=sites[i].onClickCallback;}else if(includeScript){onClickAction='loadJavasScript(\''+url+'\');return false;';}else{onClickAction='window.open(socializeit.getURL(\''+site+'\'), \'sharer\', \'toolbar=0,status=0,width='+windowWidth+',height='+windowHeight+',resizable=yes,scrollbars=yes\');return false;';}
html+='<li>'+
'<a href="#share_on_'+site+'" onclick="'+onClickAction+'" class="socialIcon" style="color:'+sites[i].color+';white-space:nowrap;">'+
'<i style="margin: 4px 0; width: 1.1em; display: inline-block; vertical-align:middle; color:'+sites[i].color+'" class="'+sites[i].iconClassName+'"></i>';if(!options.display_variant){html+='<span style="font-size: 15px; display: inline-block; vertical-align: middle; margin: 0 4px;">'+site+'</span>';}
html+='</a></li>';}
html+='</ul>';if(options.payItText&&!options.display_variant){html+='<div style="background:'+options.szColor+';padding:0.5em;margin:2px;"><p style="font-size:16px;text-align:center;margin-top:0;margin-bottom:0;"><a href="#pay_it_forward" onclick="socializeit.payItForward();return false;">'+
...
html+='</div>';var head=d.querySelector('head');var css=d.createElement('style');css.type="text/css";css.innerHTML=''+
...
'font-size: 30px;'+
'}';head.appendChild(css);}else{html='<table class="'+socit_class+'" align="center" style="'+inlineStyles+'"><tbody><tr>';if(options.display_variant){html+='<td class="shareHeader">'+options.share_sticky_label+'<td>';}else{html+='<td '+rowspan+' style="white-space:nowrap;"><strong>'+options.share_horizontal_label+'&nbsp;</strong>';if(options.whatIsThisUrl){html+='<br><div id="whatsthis" style="font-size:11px;"><a href="#" onclick="window.open\(\''+options.whatIsThisUrl+'\',\'sharer\',\'toolbar=0,status=0,width=700,height=500,resizable=yes,scrollbars=yes\'\); return false;">'+options.whatIsThisLabel+'</a></div>';}
html+='</td>';}
for(var i=0,ilen=sites.length;i<ilen;i++){var site=sites[i].name;var url=sites[i].url;var windowHeight=sites[i].windowHeight||500;var windowWidth=sites[i].windowWidth||700;var includeScript=sites[i].includeScript;var onClickAction;if(sites[i].onClickCallback){onClickAction=sites[i].onClickCallback;}else if(includeScript){onClickAction='loadJavasScript(\''+url+'\');return false;';}else{onClickAction='window.open(socializeit.getURL(\''+site+'\'), \'sharer\', \'toolbar=0,status=0,width='+windowWidth+',height='+windowHeight+',resizable=yes,scrollbars=yes\');return false;';}
html+='<td><a href="#share_on_'+site+'" onclick="'+onClickAction+'" class="socialIcon" style="color:'+sites[i].color+'; white-space:nowrap;"><i style="display: inline-block; vertical-align:middle; color:'+sites[i].color+'" class="'+sites[i].iconClassName+'"></i>';if(!options.display_variant){html+='<span style="margin-left:5px; display: inline-block; vertical-align: middle;">'+site+'</span>';}
html+='</td>';if((i+1)%buttonsInRow===0){html+='</tr><tr>';}}
html+='</tr>';if(options.payItText&&!options.display_variant){html+='<tr><td colspan="'+colspan+'" style="background:'+options.szColor+';padding:1em;"><p style="text-align:center;margin-top:0;margin-bottom:0;"><a href="#pay_it_forward" onclick="socializeit.payItForward();return false;">'+
...
'}'+
'';head.appendChild(css);if(options.version===2){var head=d.querySelector('head');var css=d.createElement('style');css.type="text/css";css.innerHTML=''+
...
var ie9css=d.createElement('div');ie9css.innerHTML='<!--[if IE 9]>'+
'<style type="text/css">'+
'.stickySidebarLeft,'+
'.stickySidebarRight {'+
'width: 37px;'+
...
'}'+
'</style>'+
'<![endif]-->';head.appendChild(ie9css);}};function loadStyleSheet(path,fn,scope){var head=document.getElementsByTagName('head')[0],link=document.createElement('link');link.setAttribute('href',path);link.setAttribute('rel','stylesheet');link.setAttribute('type','text/css');var sheet,cssRules;if('sheet'in link){sheet='sheet';cssRules='cssRules';}
else{sheet='styleSheet';cssRules='rules';}
head.appendChild(link);return link;}
function loadJavasScript(url){var script=document.createElement('script');script.setAttribute("type","text/javascript");script.setAttribute("src",url);document.getElementsByTagName("head")[0].appendChild(script);}
function payItForwardDefault(){return '<p>Would you prefer to share this page with others by linking to it?</p>'+
'<ol>'+
'<li>Click on the HTML link code below.</li>'+
'<li>Copy and paste it, adding a note of your own, into your blog, a Web page, forums, a blog comment, '+
'your Facebook account, or anywhere that someone would find this page valuable.</li>'+
'</ol>';}
var socializeit_options=socializeit_options||[];for(var i=0;i<socializeit_options.length;i++){socializeit(socializeit_options[i]);}
